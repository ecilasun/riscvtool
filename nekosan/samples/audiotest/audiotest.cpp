...
int main()
{
    ...
    while(1)
    {
        ...
        // Direct audio output:
        // 1) This write goes to a FIFO of 1024 stereo samples in size
        // 2) Output unit pulls one entry from FIFO if it is done playing previous sample
        // 3) CPU bus will stall if the FIFO has 1024 stereo samples in it
        // This effectively makes it unnecessary to use any timers etc
        // when writing to the FIFO and sound will always be generated at
        // correct clock speed.
        *IO_AudioFIFO = ((R&0xFFFF)<<16) | (L&0xFFFF);

        // APU driven audio output:
        // 1) The APU can read from the ARAM area
        // 2) APU can accept a DMA command to copy from ARAM to output FIFO
        // This mode allows the CPU to become free for other tasks as the
        // audio playback can be handled entirely by the APU.
        // If sample packet size is small enough, APU can be driven in
        // a way to allow interleaving several different types of commands
        // such as volume control and other DSP effects.
        //*IO_APUFIFO = ((R&0xFFFF)<<16) | 0x0000; // CmdWriteSamplePtrHigh
        //*IO_APUFIFO = ((L&0xFFFF)<<16) | 0x0010; // CmdWriteSamplePtrLow
