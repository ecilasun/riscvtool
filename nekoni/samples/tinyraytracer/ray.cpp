...
void render(const std::vector<Sphere> &spheres, const std::vector<Light> &lights) {
    ...
    // Error terms for each 32x32 tile
    int errR[10*7], errG[10*7], errB[10*7];
    __builtin_memset(errR, 0, sizeof(int)*10*7);
    __builtin_memset(errG, 0, sizeof(int)*10*7);
    __builtin_memset(errB, 0, sizeof(int)*10*7);

    for (size_t j = 0; j<height; j++)
    {
        // Error diffusion is per-scanline in hardware
        for (size_t i = 0; i<width; i+=4)
        {
            ...
            uint32_t addr = tileoffset | localoffset;

            uint32_t tileid = tilex+tiley*10;

            uint32_t val[4];
            for (int p = 0; p<4; ++p)
            {
                float dir_x =  (i+p + 0.5f) - width/2.f;
                ...
                // Apply error diffusion dither
                int R = std::min(255,std::max(0,int(255.f*V[0])+errR[tileid]));
                int G = std::min(255,std::max(0,int(255.f*V[1])+errG[tileid]));
                int B = std::min(255,std::max(0,int(255.f*V[2])+errB[tileid]));
                int qR = R / 32;
                int qG = G / 32;
                int qB = B / 64;

                // Error to propagate to next pixel
                errR[tileid] = R-(qR<<5);
                errG[tileid] = G-(qG<<5);
                errB[tileid] = B-(qB<<6);
            ...
            // GPU allows for masked byte writes, but we can just write 4 at once for convenience
            uint32_t pixels = (val[3]<<24)|(val[2]<<16)|(val[1]<<8)|val[0];
            GPUSetRegister(2, pixels);
            GPUSetRegister(3, addr);
...
int main()
{
    ...
    for (int r=0;r<8;++r)
    {
        ...
        GPUSetRegister(1, target);
        GPUSetRegister(2, color);
        GPUSetPaletteEntry(1, 2);
        ++target;
    }
