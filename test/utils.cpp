...
void PrintDMA(const int col, const int row, const char *message)
{
   int i=0;
   while (message[i] != 0)
   {
      ...
      for (int y=0;y<8;++y)
      {
         ...
         uint32_t dmacount = 2;
         GPUFIFO[4] = GPUOPCODE(GPUSYSDMA, 4, 5, ((dmacount&0x3FFF) | 0x4000)); // Zero-masked
      }
      ++i;
   }
}
...
void PrintDMA(const int col, const int row, const int maxlen, const char *message)
{
   ...
   while (i<maxlen && message[i] != 0)
   {
      ...
      for (int y=0;y<8;++y)
      {
         ...
         // Length of copy in DWORDs
         uint32_t dmacount = 2;
         GPUFIFO[4] = GPUOPCODE(GPUSYSDMA, 4, 5, ((dmacount&0x3FFF) | 0x4000));
      }
      ++i;
   }
}
