...
volatile uint32_t *IO_APUFIFO = (volatile uint32_t* )0x80000020;           // APU control FIFO
...
volatile uint32_t *DDR3Start = (uint32_t* )0x00000000;      // Start of DDR3 RAM region, 0x00000000 - 0x0x0FFFFFFF (256Mbytes)
volatile uint32_t *FastRAMStart = (uint32_t* )0x10000000;   // Start of Fast RAM region, 0x10000000 - 0x1003FFFF (256KBytes)
volatile uint32_t *AudioRAMStart = (uint32_t* )0x20000000;      // Start of Audio RAM region, 0x20000000 - 0x20002000 (8KBytes, 32 bit r/w access only)
...
void InitFont()
{
   ...
   //const uint8_t font[] __attribute__((aligned(4))) = {};
   // target addresss is somewhere between 0x10000000-0x1003FFFF
}
...
extern "C" {
   ...
   {
      int fh = pf_open(name); // NOTE: mode is always rb
      if (fh==-1)
         errno = ENOENT;
      return fh;
   }
