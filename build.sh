...
riscv64-unknown-elf-g++ -o ROM_nekoichi.elf test/ROM_nekoichi.cpp test/utils.cpp test/SDCARD.cpp test/FAT.cpp -std=c++11 -Wall -Ofast -march=rv32im -mabi=ilp32 -ffunction-sections -fdata-sections -Wl,-gc-sections -fPIC -lgcc -nostartfiles -Wl,-Ttest/ROM_nekoichi.lds
...
#riscv64-unknown-elf-gcc -o mandelbrot.elf test/mandelbrot.cpp test/utils.cpp -std=c++11 -ffreestanding -Wall -Ofast -march=rv32imf -mabi=ilp32f -fPIC -lgcc -lgloss -lm
riscv64-unknown-elf-g++ -o mandelbrot.elf test/mandelbrot.cpp test/utils.cpp -mexplicit-relocs -std=c++11 -Wall -Ofast -march=rv32imf -mabi=ilp32f -static -mcmodel=medany -fvisibility=hidden -nostartfiles -fPIC -ffunction-sections -fdata-sections -Wl,-gc-sections -Wl,--strip-debug -lc -lm -lgcc -lgloss -Wl,-Ttest/APP_nekoichi.lds
